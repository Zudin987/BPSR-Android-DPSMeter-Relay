...
$script:lblAddressHint = $addressHint
...
$script:step1Badge = New-StepBadge -Parent $step1
...
$script:step2Badge = New-StepBadge -Parent $step2
...
$script:step3Badge = New-StepBadge -Parent $step3
...
$script:step4Badge = New-StepBadge -Parent $step4
...
$script:step5Badge = New-StepBadge -Parent $step5
...
$statusCard = New-UiCard -X 0 -Y 0 -Width 316 -Height 215
...
$nextCard = New-UiCard -X 0 -Y 227 -Width 316 -Height 100
...
$btnFolderDetails.Add_Click({ Invoke-OpenProfileFolderGuided })
...
$detailsNote.Text = 'Closing while the relay is active asks whether to stop it or intentionally keep it running.'
...
$androidLeft.Text = "1. Phone + PC: same trusted Wi-Fi/router.`r`n`r`n2. PC: choose the matching Wi-Fi/Ethernet.`r`n`r`n3. PC: Prepare Relay.`r`n`r`n4. PC: Allow Firewall.`r`n`r`n5. PC: Set Up Phone."
...
$androidRight.Text = "6. Phone: open SFA and scan the QR.`r`n`r`n7. SFA: Per-app proxy > select BPSR only.`r`n`r`n8. PC: click Phone Ready, then Start Relay.`r`n`r`n9. Phone: Start SFA, then open BPSR."
...
if ($env:BPSR_RELAY_UI_SELF_TEST -eq '1') {
    ...
    Assert-LabelFits -Label $script:lblAddressHint -Name 'Selected network hint'
    ...
    foreach ($needle in @('8. PC: click Phone Ready, then Start Relay.','9. Phone: Start SFA, then open BPSR.')) {
    ...
    Write-Host 'UI SELF-TEST PASS: layout fits, guided step states/phone confirmation/close-stop guardrails exist, startup order is explicit, StarSEA target remains visible.'
    return
}
