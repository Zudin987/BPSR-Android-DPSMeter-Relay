...
# Engine: bump RC and expose the same foreign-process detection logic to the UI.
Replace-Exact -Path 'scripts\BPSRRelayManager.ps1' -Old "$ManagerVersion = '1.0.0-rc.11'" -New "$ManagerVersion = '1.0.0-rc.12'"
...
# UI: make buttons paint predictably and center text explicitly.
$oldButton = @'
...
# Add beginner-friendly old-relay identification, confirmation, and safe close.
$anchor = @'
...
function Get-FriendlyErrorText {
'@
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old $anchor -New $helpers

# Show exact process names for foreign-relay errors and offer one-click cleanup.
$oldShow = @'
...
# Use the engine's exact foreign process list in Home status.
$oldForeign = @'
...
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old "        $script:lblNextAction.Text = 'Close old relay apps or restart your PC.'" -New @'
        if ($foreignProcesses.Count -eq 1) {
...
# Fix compact-card button hierarchy: title on row 1, action button on row 2.
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old "$script:btnSetup = New-UiButton -Text 'Prepare Relay' -X 382 -Y 17 -Width 148 -Height 36 -Primary" -New "$script:btnSetup = New-UiButton -Text 'Prepare Relay' -X 382 -Y 32 -Width 148 -Height 30 -Primary"
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old "$script:btnSetup.Add_Click({ try { Setup-Relay } catch { Show-FriendlyError -Title 'Could not prepare relay' -Exception $_.Exception } })" -New "$script:btnSetup.Add_Click({ Invoke-PrepareRelay })"
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old "$script:btnFirewall = New-UiButton -Text 'Allow Firewall' -X 382 -Y 17 -Width 148 -Height 36" -New "$script:btnFirewall = New-UiButton -Text 'Allow Firewall' -X 382 -Y 32 -Width 148 -Height 30"
Replace-Exact -Path 'scripts\ManagerUi.ps1' -Old "$btnCopyDpsHome = New-UiButton -Text 'Copy Setup Notes' -X 382 -Y 17 -Width 148 -Height 36" -New "$btnCopyDpsHome = New-UiButton -Text 'Copy Setup Notes' -X 382 -Y 32 -Width 148 -Height 30"

Write-Host 'RC.12 UX patch applied.'
