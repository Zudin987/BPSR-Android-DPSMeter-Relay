...
function Replace-FunctionBlock {
    ...
    $pattern = '(?ms)^function ' + [regex]::Escape($Name) + ' \{.*?(?=^function ' + [regex]::Escape($NextName) + ' \{)'
    $matches = [regex]::Matches($Text.Value, $pattern)
    if ($matches.Count -ne 1) { throw ("Function $Name boundary expected once; found " + $matches.Count) }
    $Text.Value = [regex]::Replace($Text.Value, $pattern, ($NewBlock.TrimEnd() + "`r`n`r`n"), 1)
}
...
Replace-Literal $m '$ShareLifetimeSeconds = 300' "$ShareLifetimeSeconds = 300`r`n$MaxLogBytes = 2097152" 'log limit constant'
...
Replace-Literal $m "    Stop-Relay`r`n    $checks = Get-PreflightChecks -PcIp $pcIp -RequirePortFree" "    Stop-Relay`r`n    [void](Ensure-InternalBridgePortAvailable -PcIp $pcIp)`r`n    $checks = Get-PreflightChecks -PcIp $pcIp -RequirePortFree" 'refresh stale internal port before start'
...
Replace-Literal $m "Android profile' 'OK' ('v1.0.0 v4-compatible profile matches '" "Android profile' 'OK' ('v1.0.1 v4-compatible profile matches '" 'preflight profile version'
Replace-Literal $m "Android profile' 'FAIL' 'v1.0.0 profile is not generated." "Android profile' 'FAIL' 'v1.0.1 profile is not generated." 'preflight profile missing version'
...
Replace-Literal $m "Generated v4-compatible Android SFA profile for PC ' + $PcIp + '. Re-import is required when upgrading from RC.14." "Generated v4-compatible Android SFA profile for PC ' + $PcIp + '. Re-import is required only when the profile/IP changes or when upgrading from an incompatible test profile.'" 'profile generated log'
...
elif ($script:cmbIp.Items.Count -gt 0) { $script:cmbIp.SelectedIndex = 0 }
'@
# PowerShell uses elseif, not elif.
$newSelection = $newSelection.Replace('elif', 'elseif')
...
# Parse all changed PowerShell before committing.
